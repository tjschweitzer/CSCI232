...
public class AutocompleteGUI extends JFrame {
    ...
    public AutocompleteGUI(String filename, int k) {
        ...
        layout.setVerticalGroup(layout.createSequentialGroup()
                .addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING)
                        .addGroup(layout.createSequentialGroup().addComponent(textLabel).addComponent(checkbox))
    ...
    private class AutocompletePanel extends JPanel {
        ...
        //// private JList<String> suggestions; // a list of autocomplete matches (Java
        //// 7)
            ...
            if (text.equals("")) {
                ...
                if (Math.min(k, allResults.length) > 0) {
                    for (int i = 0; i < results.length; i++) {
                        ...
                        if (displayWeights) {
                            results[i] += "<td width=\"10%\" align=right>" + "<font size=-1><span id=\"weight\" "
     ...
     * 
     * @param args the filename from which the Autocomplete object is populated and
     *             the integer k which defines the maximum number of objects in the
     *             dropdown menu
     */
